...
function stripBoiler(s){
  ...
  // de-link markdown links -> keep anchor text
  s = s.replace(/\[([^\]]+)\]\([^)]*\)/g, "$1");
...
const vitals={lcpMs:2000,cls:0.05,inpMs:120};
...
for(const pth of all){ if(pth.endsWith(`${tenant}/${slug}/index.mdx`)) continue; const jac=jaccard(txt, fs.readFileSync(pth,"utf8"), THRESH_SIM_N); if(jac>similarityMax) similarityMax=jac; }
...
console.log(JSON.stringify({tenant,slug,qc,allowHits},null,2));
